...
/**
 * @title DODOIntegration
 * @notice Integration with DODO Protocol for PMM pools and advanced trading
 * @dev Provides access to DODO's Proactive Market Maker pools
 */
contract DODOIntegration is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // DODO Protocol addresses
    address public constant DODO_APPROVE = 0xA867241cDC8d3b0C07C85cC06F25a0cD3b5474d8;
    address public constant DODO_PROXY = 0x6dF5B0C377518F5E9e2c96682a8D3149b31399AB;
    ...
    event PoolAdded(address indexed pool, bool supported);
    ...
    constructor() Ownable(msg.sender) {
        ...
        poolFee = 30; // 0.3% pool fee
        authorizedOperators[msg.sender] = true;
    }

    /**
     * @notice Execute a swap using DODO Protocol
     * @param tokenIn Input token address
     * @param tokenOut Output token address
     * @param amountIn Amount of input tokens
     * @param minAmountOut Minimum amount of output tokens
     * @param pool DODO pool address
    ...
    function executeSwap(
        ...
        require(supportedPools[pool], "Pool not supported");
        ...
        // Approve DODO proxy
        IERC20(tokenIn).approve(DODO_PROXY, amountIn);
        ...
        try this.performSwap(swapData) {
            ...
            emit SwapExecuted(tokenIn, tokenOut, amountIn, amountOut, pool, swapId);
            
        } catch (bytes memory reason) {
            // Revert approval
            IERC20(tokenIn).approve(DODO_PROXY, 0);
    ...
    /**
     * @notice Execute a swap with DODO proxy (external function for try/catch)
     * @param swapData DODO swap data
     */
    function performSwap(bytes calldata swapData) external {
        ...
        // Call DODO proxy
        (bool success, bytes memory result) = DODO_PROXY.call(swapData);
    ...
    /**
     * @notice Get pool information from DODO
     * @param pool DODO pool address
    ...
    function getPoolInfo(address pool) external view returns (
        ...
        // This would typically call DODO pool contract
        // For now, return placeholder values
        return (address(0), address(0), 0, 0);
    }

    /**
     * @notice Get optimal swap route from DODO
     * @param tokenIn Input token address
     * @param tokenOut Output token address
     * @param amountIn Amount of input tokens
     * @return pool Optimal pool address
    ...
    function getOptimalRoute(
        ...
        address tokenOut,
        uint256 amountIn
    ) external view returns (address pool, bytes memory route) {
        // This would typically query DODO API or pool contracts
        // For now, return empty values as placeholder
        return (address(0), "");
    }
    ...
    function calculateExpectedAmount(
        ...
        // This would typically query DODO pool reserves or price oracles
    ...
    /**
     * @notice Add supported DODO pool
    ...
    /**
     * @notice Remove supported DODO pool
     ...
     * @return supported Whether pool is supported
     */
    function isPoolSupported(address pool) external view returns (bool supported) {
     ...
     * @param newFee New pool fee in basis points
     */
    function updatePoolFee(uint256 newFee) external onlyOwner {
        require(newFee <= 500, "Fee too high"); // Max 5%
        uint256 oldFee = poolFee;
        poolFee = newFee;
